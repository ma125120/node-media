<template>
  <div class="">
    <div class="flex">
      ...
      <n-button type="plain" :loading="loading" @click="getInfo">查询</n-button>
      <n-button type="primary" :loading="loading" @click="format">转码</n-button>
    </div>
    <div class="area">
...
export default {
  setup() {
    ...
    const getInfo = async () => {
      state.loading = true
      const res: any = await video.info({ url: state.url }).catch(err => {
        state.loading = false
      });
      state.loading = false
      if (!res) return ;
